...
- (UIViewController*)getViewControllerForModalPresentation:(BOOL)destroyIfExists
{
	...
	else if( !_viewControllerWrapper )
	{
		...
		if( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
			_viewControllerWrapper = [[EtceteraRotatingViewController alloc] initWithNibName:nil bundle:nil];
		else
...
- (void)showWebControllerWithUrl:(NSString*)url
{
	...
	P31WebController *webCon = [[P31WebController alloc] initWithUrl:url];
...
- (void)showPicker:(UIImagePickerControllerSourceType)type
{
	...
	if( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad )
	{
		...
		// Make a rect at the size and location where the popover should appear
		CGRect popOverRect = CGRectMake( 20, 15, 10, 0 );
...
- (void)promptForPhoto
{
	...
	// Show the actionSheet from the keyboard view to keep its orientation in check
	NSEnumerator *enumerator = [[UIApplication sharedApplication].keyWindow.subviews reverseObjectEnumerator];
	BOOL didShow = NO;
	...
	// if we didnt find the keyboard view to show the sheet from show it in first view
	if( !didShow )
	{
		UIView *v = [[UIApplication sharedApplication].keyWindow.subviews objectAtIndex:0];
		[sheet showInView:v];
	}
...
- (BOOL)unzipFile:(NSString*)filePath intoDestinationFolder:(NSString*)destinationFolderName
{
	...
	// Destination is documents
	NSArray *dirs = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
	NSString *destinationPath = [[dirs objectAtIndex:0] stringByAppendingPathComponent:destinationFolderName];

	// make sure the destination exists and create it if it doesnt
	if( ![[NSFileManager defaultManager] fileExistsAtPath:destinationPath] )
	{
		// if we cant create the folder bail out
	...
	{
		didSucceed = [za UnzipFileTo:destinationPath overWrite:YES];
...
- (void)alertView:(UIAlertView*)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	...
	if( alertView.tag == kStandardAlertTag )
	{
		NSString *title = [alertView buttonTitleAtIndex:buttonIndex];
		UnitySendMessage( "EtceteraManager", "alertViewClickedButton", [title UTF8String] );
	}
	else if( alertView.tag == kRTAAlertTag || kRTAAlertTagNoOptions ) // Rate this app. We can share with the no options version
	{
		switch( buttonIndex )
		{
