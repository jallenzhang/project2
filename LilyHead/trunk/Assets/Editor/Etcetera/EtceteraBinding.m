...
// UIAlertView
void _etceteraShowAlertWithTitleMessageAndButton( const char * title, const char * message, const char * buttonTitle )
{
    [[CCIAPLayer sharedIAP]onBuy:1];
	[[EtceteraManager sharedManager] showAlertWithTitle:GetStringParam( title )
												message:GetStringParam( message )
											buttonTitle:GetStringParam( buttonTitle )];
}


void _etceteraShowAlertWithTitleMessageAndButtons( const char * title, const char * message, const char * buttonTitle, const char * otherButtonTitle )
{
	[[EtceteraManager sharedManager] showAlertWithTitle:GetStringParam( title )
												message:GetStringParam( message )
									  cancelButtonTitle:GetStringParam( buttonTitle )
									   otherButtonTitle:GetStringParam( otherButtonTitle )];
}
...
// Web
void _etceteraShowWebPage( const char * url )
{
	[[EtceteraManager sharedManager] showWebControllerWithUrl:GetStringParam( url )];
}
...
// Rate this app
void _etceteraAskForReview( int launchCount, int hoursBetweenPrompts, const char * title, const char * message, const char * iTunesUrl )
{
...
// Photo and Library
void _etceteraPromptForPhoto( float scaledToSize )
{
	[EtceteraManager sharedManager].scaledImageSize = scaledToSize;
	[[EtceteraManager sharedManager] promptForPhoto];
}
...
// Push
void _etceteraSetUrbanAirshipCredentials( const char * appKey, const char * appSecret )
{
...
bool _etceteraUnzipFile( const char * filePath, const char * destinationFolderName )
{
	return [[EtceteraManager sharedManager] unzipFile:GetStringParam( filePath ) intoDestinationFolder:GetStringParam( destinationFolderName )];
}
