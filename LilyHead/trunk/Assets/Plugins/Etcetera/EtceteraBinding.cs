...
public class EtceteraBinding
{
    ...
    */
	...
	#region CT purchase
	[DllImport("__Internal")]
	private static extern void _etceteraPurchaseInfos();
	
	public static void etceteraPurchaseInfos()
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
			_etceteraPurchaseInfos();
	}
	
	[DllImport("__Internal")]
	private static extern void _etceteraUpdatePurchaseInfos(string info);
	
	public static void etceteraUpdatePurchaseInfos(string info)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
			_etceteraUpdatePurchaseInfos(info);
	}
	
	[DllImport("__Internal")]
    private static extern void _etceteraPurchaseWithIndex( int index );
	
	public static void etceteraPurchaseWithIndex(int index)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
			_etceteraPurchaseWithIndex(index);
	}
	
	[DllImport("__Internal")]
    private static extern void UnityCallAlixPay( string title, string body, string price, string userId );
	
	public static void etceteraPurchaseWithUnityCallAlixPay(string title, long price, string userId, string channelId)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer)
			UnityCallAlixPay(title, string.Empty, price.ToString(), userId + "_" + channelId.Replace('.', '_'));
	}
	
	[DllImport("__Internal")]
    private static extern void UnityCall91Pay( string title, string body, string price, string userId );
	
	public static void etceteraPurchaseWithUnityCall91Pay(string title, long price, string userId, string channelId)
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer)
			UnityCall91Pay(title, string.Empty, price.ToString(), userId + "_" + channelId.Replace('.', '_'));
	}
    ...
    [DllImport("__Internal")]
    private static extern void _etceteraShowWebPage( string url );
 
	// Opens a web view with the url
    public static void showWebPage( string url )
    {
        if( Application.platform == RuntimePlatform.IPhonePlayer )
			_etceteraShowWebPage( url );
    }
	
	public static void ShowWebView(string url,string gameObjectName,string methodName)
	{
		if(Application.platform==RuntimePlatform.IPhonePlayer)
		{
			showWebView(url,gameObjectName,methodName);
		}
	}
	
	[DllImport("__Internal")]
	public static extern void showWebView(string url,string gameObjectName,string methodName);
	
	[DllImport("__Internal")]
	public static extern void dismissWrappedController();
	
	[DllImport("__Internal")]
	public static extern void excuteJS(string foo);
	...
	#region device info
	[DllImport("__Internal")]
	public static extern string getDeviceVersion();
	[DllImport("__Internal")]
