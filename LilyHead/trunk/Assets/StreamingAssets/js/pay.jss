...
$(function () {
    $('#alipay').bind('touchstart', function () {
		BtnAliPayPressEffect(1);
    }).bind('touchend', function () {
		BtnAliPayPressEffect(0);
		CallAliPay();
        CloseView();
    });

    $('#yeepay').bind('touchstart', function () {
		BtnYeePayPressEffect(1);
    }).bind('touchend', function () {
		BtnYeePayPressEffect(0);
		CallYeePay();
        CloseView();
    });

    $('#close').bind('touchstart', function () {
        BtnClosePressEffect(1);
    }).bind('touchend', function () {
		BtnClosePressEffect(0);
        CloseView();
    });
...
function CallAliPay() {
    window.WebBinding.Submit("Alipay");
}

function CallYeePay() {
    window.WebBinding.Submit("Yeepay");
}
...
function PrintName(){
	var mynickname = window.WebBinding.GetNickName();
	alert(mynickname);
	document.getElementById("nickname").innerHTML = mynickname;
}

function BtnAliPayPressEffect(isPress){
	if(isPress == 1)
		setImgSrc("idAliPay", "image/pay_CenterBtn_1_press.gif");
	else 
		setImgSrc("idAliPay", "image/pay_CenterBtn_1.png");
}

function BtnYeePayPressEffect(isPress){
	if(isPress == 1)
		setImgSrc("idYeePay", "image/pay_CenterBtn_2_press.png");
	else
		setImgSrc("idYeePay", "image/pay_CenterBtn_2.png");
}

function BtnClosePressEffect(isPress){
	if(isPress == 1)
		setImgSrc("idCloseBtn", "image/pay_Topbg_btn_press.png");
	else 
		setImgSrc("idCloseBtn", "image/pay_Topbg_btn.png");
}
