...
$(function () {
    $('#sendEmail').bind('touchend', function () {
		sendEmail();
    });

	$('#btnBack').bind('touchend', function () {
		backToLoginPage();
    });
});

function unitySendMsg(msg){
	window.alert(msg);
}

function sendEmail(){
	var my_mail = $("#tfEmail").val();
	if (navigator.userAgent.match(/android/i)) {
        window.WebBinding.Submit(my_mail, "CallBackSendEmailMethod");
    }
    else {
        window.location = '/ForgetPwd/' + my_mail;
    }	
}

function backToLoginPage(){
	// window.WebBinding.Submit("","CallLoginPageMethod");
	if(isAndroidOrIOS()){
		window.location.href="file:///android_asset/Login_android.html";
	}else{
		// window.location.href="file:////var/mobile/Applications/796D2563-433B-4C32-8DC0-073D2DA5C8F7/lilypoker.app/Data/Raw/Login_ios.html";
		window.location = '/CallLoginPage/';
	}
}
