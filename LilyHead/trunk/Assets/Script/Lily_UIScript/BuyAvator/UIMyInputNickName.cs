...
public class UIMyInputNickName : MonoBehaviour
{
	...
	static public UIMyInputNickName current;
		...
		if (label != null && enabled && gameObject.active)
		{
			if (isSelected)
			{
					...
					Application.platform == RuntimePlatform.Android)
				{
					...
					{
						mKeyboard = TouchScreenKeyboard.Open(mText, (TouchScreenKeyboardType)((int)type),false,isPassword,false);
					}
#endif
				}
				else
	...
	void UpdateLabel ()
	{
		...
		{
			Debug.Log(mText);
			mText = AssemblyCSharp.UtilityHelper.checkNickName(mText,maxChars);
		}
