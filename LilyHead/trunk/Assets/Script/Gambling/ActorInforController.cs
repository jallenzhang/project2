...
public class WonPot
{
	...
	public WonPot(int pot,int[] win,long[] winat,int[] att)
	{
		potId=pot;
		winner=win;
		winamnt=winat;
...
public class KickFoldTeiceDialog : DialogInfo {
	public KickFoldTeiceDialog()
	{
		
		this.Title = LocalizeHelper.Translate("KICKFOLDTWICE_TITLE");
		this.Description = LocalizeHelper.Translate("KICKFOLDTWICE_DESCRIPTION");
		this.Buttons = 1;
	}
	
	public override void Process()
	{
 		//ActorInforController.Singleton.GotoRoom();//.DidLeaveGameFinished();
		
		if(Util.isMatch())
		{
			MatchInforController.Singleton.DidLeaveGameFinished();//.leaveGame();
		} 
		else
 			ActorInforController.Singleton.DidLeaveGameFinished();//.leaveGame();
	}
}
public class CardFriendNotHereDialog : DialogInfo {
	public CardFriendNotHereDialog()
	{
		this.Title = LocalizeHelper.Translate("CARDFRIEND_NOTHERE_ERROT");
		this.Description = LocalizeHelper.Translate("CARDFRIEND_NOTHERE_DESCRPITION");
		this.Buttons = 1;
	}
...
public class ActorInforController : MonoBehaviour {
	
	 
 	AsyncOperation async = null;
	
	public static bool hasJoined=false;
	public static ActorInforController Singleton {get;private set;}
...
//						#endif
	...
	void DoPckerPlayerLeave(ActorInfor ac,bool isKick,PlayerLeaveType leavetype)
	{
 		if(isKick)
		{
			if(leavetype == PlayerLeaveType.KickFoldTwice &&  ac.name ==User.Singleton.UserData.Name)
			{
			    PopKickFodTwice();
			}
			else if(leavetype == PlayerLeaveType.KickByPlayer &&  ac.name ==User.Singleton.UserData.Name)
			{
			    DidLeaveGameFinished();
			}
		    else 
			{
				if(ac.name == User.Singleton.UserData.Name)
				{
					DidLeaveGameFinished();
				}
			}
...
//		//Debug.Log("DoPockerPlayerHoleCardsChangedEvent");
//		return;
		if(players.Count>0)
		{
	 		...
	 		for(int i=0 ;i<players.Count*2;i++)
			{
				...
				if(ac!=null)
				{
					...
					if(User.Singleton.UserData.NoSeat==ac.NoSeat)
					{
						...
						if(i<players.Count)
						{
							handcardsprite.waitForRotateTime=0.15f*(players.Count>1?(players.Count-1):1);
						}
					}
					else
					{
...
//						}
					...
					{
						if(infoitem.MoneySafeAmnt<info.BigBlindAmnt)
						{
	...
	void DopockerBetTurnBegan()
	{
		...
		{
	 		TableInfo info=Room.Singleton.PokerGame.TableInfo;
			//Debug.LogWarning("DidBetTurnStartEvent");
...
//				{
//					PlayerInfo item=info.Players[i];
//					Debug.LogWarning("item "+item.Name+" "+item.NoSeat);
//				}
				hasJoinGamed=true;
	 
 				if(info.TotalPotAmnt>0)
				 	 Util.SetLabelValue(totalchipbg.transform,"wonpottotalchip",Util.getLableMoneyK_MModel(info.TotalPotAmnt));
				...
				if(gamestate ==TypeState.Playing )
				{
					...
					if(iteminfo!=null)
					{
						if(!(iteminfo.IsPlaying==true || iteminfo.IsAllIn==true) && NeedToShowTheViewWaitingForNext==false)
						{ 
							ShowTheViewWaitingForNext();
							NeedToShowTheViewWaitingForNext=true;
	 					}
					}
  				}
	...
	void Start () {
		...
		ShowSignal();
		TableState.Singleton.CheckInTableInfoState(TableState.GameTableType.GameTableType_Normal);
	...
	{   
		hasJoined=true;
		ActorInforController.Singleton=this;
...
//		{
	...
	void OnDestroy()
	{
		...
		ActorInforController.Singleton=null;
	}
...
//		}
	...
	void addMakeFriend(GameObject btn)
	{
		...
		if(infor!=null)
		{
			...
			Debug.LogWarning("playerName "+playerName+" "+Noseat +" "+User.Singleton.UserData.NoSeat+" " +User.Singleton.UserData.Name);
