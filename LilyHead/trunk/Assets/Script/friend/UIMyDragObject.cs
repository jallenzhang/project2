...
public class UIMyDragObject : IgnoreTimeScale
{
	...
	private Vector3 myVect = Vector3.zero;
	
	public delegate void OnDragFinished (Vector3 OffsetPosition,Vector3 mLastPos,UIMyDragObject drag);
	public OnDragFinished onDragFinished;
	
	public bool NeedDampen = true;
	...
	void OnPress (bool pressed)
	{
		if (enabled && gameObject.active && target != null)
		{
			...
			if (pressed)
			{
				...
				if(momentumAmount> 0f&&!NeedDampen)
					momentumAmount = - momentumAmount;
				if(NeedDampen)
					momentumAmount = Mathf.Abs(momentumAmount);
	...
	void OnDrag (Vector2 delta)
	{
		if (enabled && gameObject.active && target != null)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				#region Add for the count < 5
				if(myVect!=Vector3.zero&&!NeedDampen)
				{
					if(myVect.y > mLastPos.y)
					{
						dragDirection = DragDirection.Down;
						if(momentumAmount > 0f)
							momentumAmount = - momentumAmount;
					}
					else
					{
