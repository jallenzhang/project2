...
namespace Photon.LoadBalancing.GameServer
{
    ...
    public class Game : LilyServer.LilyGame
    {
        ...
        //public Game(string gameId,string lobbyname, string userId)
        //    : base(gameId)
        //{
        //    Log.Debug("############### lee test game");

        //    // add game state to the master server
        //    this.AddGameStateOnMaster(gameId, userId);      
        //}

        public Game(string gameId, string lobbyname,string userId)
            : base(gameId, lobbyname)
        {

            // add game state to the master server
            if (gameId.StartsWith("sytem") || gameId.StartsWith("career"))
                this.AddGameStateOnMaster(gameId, userId);
        }
        ...
        protected override Actor HandleJoinOperation(LitePeer peer, Lite.Operations.JoinRequest joinRequest, SendParameters sendParamters) {
            if (!this.ValidateGame(peer, joinRequest.OperationRequest, sendParamters))
            {
                return null;
            }
            ...
            Actor actor = base.HandleJoinOperation(peer, baseRequest, sendParamters);
            ...
            var peerId = gamePeer.UserId ?? string.Empty;
        ...
        public virtual void UpdateGameStateOnMaster(Hashtable gameProperties, string newPeerId, string removedPeerId)
        {
            List<string> newPeers = string.IsNullOrEmpty(newPeerId) ? null : new List<string> { newPeerId };
            List<string> removedPeers = string.IsNullOrEmpty(removedPeerId) ? null : new List<string> { removedPeerId };
        ...
        public void AddGameStateOnMaster(string gameId, string userId) {
            GameApplication.Instance.MasterPeer.AddGameState(
                gameId,
                userId,
                (byte)this.Actors.Count);
        }
