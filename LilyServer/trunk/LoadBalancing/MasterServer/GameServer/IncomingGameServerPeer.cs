...
namespace Photon.LoadBalancing.MasterServer.GameServer
{
    ...
    public class IncomingGameServerPeer : ServerPeerBase
    {
        ...
        protected virtual void HandleAddGameState(IEventData eventData)
        {
            if (log.IsDebugEnabled)
            {
                log.DebugFormat("HandleAddGameState");
            }

            var addEvent = new UpdateGameEvent(this.Protocol, eventData);
            if (addEvent.IsValid == false)
            {
                string msg = addEvent.GetErrorMessage();
                log.ErrorFormat("AddGame contract error: {0}", msg);
                return;
            }
            
            this.application.Lobby.AddGameState(addEvent, this);
        }
        ...
        protected override void OnEvent(IEventData eventData, SendParameters sendParameters)
        {
            ...
            switch ((ServerEventCode)eventData.Code)
            {
                ...
                case ServerEventCode.AddGameState:
                    this.HandleAddGameState(eventData);
                    break;
            }
        }
        ...
        protected override void OnOperationResponse(OperationResponse operationResponse, SendParameters sendParameters)
        {
            ...
            switch ((OperationCode)operationResponse.OperationCode)
            {
                ...
                case OperationCode.GameHistoryActions:
                    {
                        if (operationResponse.Parameters.ContainsKey((byte)DataPersist.LilyOpKey.HistoryAction))
                        {
                            string address=this.TcpAddress;
                            operationResponse.Parameters.Add((byte)DataPersist.LilyOpKey.Address, address);                            
                        }
                        this.application.Lobby.SendHistoryActionResponse(operationResponse);
                    }
                    break;
            }
        }
        ...
        public void AskForGameHistoryActions(string gameId, int totalRounds, int stepId, int sessionId,string mail)
        {
            
            var request = new OperationRequest((byte)OperationCode.GameHistoryActions);
            request.Parameters = new System.Collections.Generic.Dictionary<byte, object>();
            request.Parameters.Add((byte)DataPersist.LilyOpKey.GameId,gameId);
            request.Parameters.Add((byte)DataPersist.LilyOpKey.TableRound, totalRounds);
            request.Parameters.Add((byte)DataPersist.LilyOpKey.StepID,stepId);
            request.Parameters.Add((byte)DataPersist.LilyOpKey.UserId,sessionId);
            request.Parameters.Add((byte)DataPersist.LilyOpKey.Mail,mail);
            //master 对应的peer.UserId;
            //request.Parameters.Add((byte)DataPersist.LilyOpKey.NodeId, nodeId);
