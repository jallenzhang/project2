...
namespace Photon.LoadBalancing.MasterServer.Lobby
{
    ...
    public class GameList
    {
        ...
        {
            //log.WarnFormat("GameList ----- list count: {0}", this.gameDict.Count);
            var toRemove = new List<GameState>();
            foreach (GameState gameState in this.gameDict)
            {
                //log.WarnFormat("GameState -----gameid:{1}, playcount peerDict: {0}; ", gameState.peerDict.Count,gameState.Id);
        ...
        //    foreach (GameState gameState in this.gameDict)
        //    {
        //        log.WarnFormat("GameState ----- playcount peerDict: {0}; ", gameState.peerDict.Count);
        //        if (gameState.peerDict.Count == 0) {
        //            toRemove.Add(gameState);
        //        }
        //    }
