...
namespace Photon.LoadBalancing
{
    ...
    public class LoadBalancer<TServer>
    {
        ...
        public bool TryGetServer(string serverId,out TServer server)
        {
            lock(this.serverList){
                server = this.serverList.FirstOrDefault(r=>(r.Key as IncomingGameServerPeer).ServerId.ToString()==serverId).Key;
                //foreach (KeyValuePair<TServer,int> item in this.serverList)
                //{
                //    TServer iserver = item.Key;
                //}
                if (server != null)
                    return true;
            }
