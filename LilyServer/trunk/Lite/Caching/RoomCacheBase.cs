...
namespace Lite.Caching
{
    ...
    public abstract class RoomCacheBase
    {
        ...
        public List<Room> getAllRooms() {
            return this.roomInstances.Select(rs => rs.Value.Room).ToList();
        }
        /// <summary>
        ...
        public void ReleaseRoomReference(RoomReference roomReference)
        {
            ...
            lock (this.syncRoot)
            {
                ...
                {
                    string roomName = roomReference.Room.Name;
                    this.ExecutionFiber.Schedule(() => this.diposeReferenceRoom(roomName), 30000);
                //    if (log.IsDebugEnabled)
                //    {
        ...
        private void diposeReferenceRoom(string name) {
            Room room = null;
            lock (this.syncRoot)
            {
                RoomInstance roomInstance;
                if (!this.roomInstances.TryGetValue(name, out roomInstance))
                {
                    return;
                }
                ...
                if (roomInstance.ReferenceCount <= 0)
                {
                    if (log.IsDebugEnabled)
                    {
                        log.DebugFormat("Removing room instance: roomId={0}", name);
                    }
