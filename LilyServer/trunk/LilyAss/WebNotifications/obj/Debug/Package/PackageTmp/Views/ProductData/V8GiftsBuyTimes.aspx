...
<h2>8 各类礼物被购买次数
   ...
   <div>
     <p>
     <% using (Html.BeginForm(new { Action = "V8GiftsBuyTimesAction" }))
     { %>
        <input type="submit" value="查询" onclick='this.form.action="<%=Url.Action("V8GiftsBuyTimesAction")%>"' />
        <input type="submit" value="导出表格" onclick='this.form.action="<%=Url.Action("DownloadV8GiftsBuyTimesAction")%>"' />
     <% } %>
     </p>
        ...
        <div class="tabbed_area">
            ...
            <div id="content_1" class="tabcontent" style="display: block; ">
        	    <% if(Model != null) { %>
                     ...
                     { %>
                        <p><label>礼物名称: <%= Html.Encode(item.Name) %></label>
                        <label>购买次数: <%= item.Count%></label>
                        </p>                
                     <% } %>
                 <% } %>
            </div>
	                ...
	                <script type="text/javascript">
	                    ...
	                    function printFlash() {
	                        ...
	                        //"http://teethgrinder.co.uk/open-flash-chart/open-flash-chart.swf",
			                    ...
			                    "9.0.0",
			                    "expressInstall.swf",
			                    { "data-file": "%2FProductData%2FGetV8GiftsBuyTimesActionChart" }
		                    );
	                    }
	                </script>
	                <div id="my_chart"></div>
