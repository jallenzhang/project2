...
<% } %>--%>
<% if (LilyPokerHome.Helper.LoginHelper.ValidateUserInSession()) { %>
    <p>
        Hello, <%: Html.ActionLink(Session["username"] as string, "ChangePassword", "Account", routeValues: null, htmlAttributes: new { @class = "username", title = "Change password" })%>!
        <%: Html.ActionLink("Log off", "LogOff", "Account") %>
    </p>
<% } else { %>
    <ul>
