...
namespace DataPersist
{
    public class TableInfo
    {
        ...
        public List<PlayerInfo> PlayingOrAllinPlayers()
        {
            return PlayingOrAllinPlayersFrom(0);
        } 
        ...
        public TableInfo(string name, int bigBlind, int nbSeats, int thinktime) 
            :this(name,bigBlind,nbSeats,TypeBet.NoLimit)
        {
            this.ThinkingTime = thinktime;
        }
        ...
        public List<PlayerInfo> PlayingOrAllinPlayersFrom(int seat)
        {
            List<PlayerInfo> list = new List<PlayerInfo>();
            for (int i = 0; i < m_NbMaxSeats; ++i)
            {
                if (m_Players[i] != null && (m_Players[i].IsPlaying||m_Players[i].IsAllIn))
                {
                    list.Add(m_Players[i]);
                }
            }
