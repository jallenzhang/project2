...
namespace PokerWorld.HandEvaluator
{
    ...
    public partial class Hand : IComparable
    {
        ...
        public static string BestFiveFromMask(ulong mask){
#if DEBUG
            if (Hand.BitCount(mask) < 5 || Hand.BitCount(mask) > 7) throw new ArgumentException();
#endif

            uint hv = Hand.Evaluate(mask);
            string fiveCards = string.Empty;
            // Loop through possible 5 card hands
            foreach (ulong hand5 in Hand.Hands(0UL, ~mask, 5))
            {
                if (Hand.Evaluate(hand5) == hv)
                {
                    fiveCards = MaskToString(hand5);
                    break;
                }
            }
            if (string.IsNullOrEmpty(fiveCards))
                throw new Exception("Error");

            Card[] arrayCard = Array.ConvertAll<string, Card>(fiveCards.Split(' '), c => new Card { Rank = Hand.CardRank(Hand.ParseCard(c)), Name = c });
            var query = arrayCard.OrderByDescending(r => r.Rank)
                                 .GroupBy(r => r.Rank, (Rank, r) => new
                                 {
                                     Key = Rank,
                                     Counts = r.Count(),
                                     Cards = r
            ...
            fiveCards = string.Empty;
            foreach (var item in query)
            {
                foreach (var c in item.Cards)
                {
                    fiveCards = fiveCards + c.Name + " ";
                }
            }

            return fiveCards.TrimEnd().Replace("T","10");
        }
